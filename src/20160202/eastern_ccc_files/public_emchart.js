; (function ($) {
    ...
    emChart.utils.addMarkLogo = function (container, enabled) {
        if (enabled) {
            $(".highcharts-container", "#" + container).append("<div class=\"highcharts-logo\" style=\"color: #909090;font-size: 10px;background: url('/js/Emcharts/js/lib/highcharts/images/mark-logo.png') no-repeat scroll 0 0;width:110px;height: 25px;fill: #909090;position:absolute;z-index:-1;\"></div>");
                ...
                if (settings.type != type.link) {
                    ...
                    $.each(result.series, function (key, value) {
                        ...
                        $.each(seriesData, function (i, v) {
                            categories.push('<a target="_blank" href="' + v.linkUrl + '">' + emChart.utils.addSpace(result.categories[i], settings.xLableSpace) + '</a>');
                        });
                    })
                }
